...
interface QualityResult {
  ...
  hasProhibitedItem: boolean;
  hasBlacklistedKeyword: boolean;
  hasRestrictedBrand: boolean;
  hasNGSellerRestriction: boolean;
  hasMAForbiddenBrand: boolean;
  hasMAFragranceOnlyBrand: boolean;
  hasMASellerRestriction: boolean;
  hasMAProhibitedBookSeller: boolean;
  hasCounterfeitFlag: boolean;
  hasSuspiciousPrice: boolean;
  hasInvalidVariation: boolean;
}
...
/**
 * Detect the user's country from browser locale/timezone and map it to
 * a supported Jumia country code. Falls back to "NG" if no match.
 */
function detectDefaultCountry(): string {
  const COUNTRY_MAP: Record<string, string> = {
    NG: "NG", KE: "KE", UG: "UG", EG: "EG",
    GH: "GH", CI: "CI", MA: "MA", TN: "TN",
    SN: "SN", DZ: "DZ", IC: "IC",
  };
  const TIMEZONE_MAP: Record<string, string> = {
    "Africa/Lagos": "NG", "Africa/Kano": "NG", "Africa/Abuja": "NG",
    "Africa/Nairobi": "KE",
    "Africa/Kampala": "UG",
    "Africa/Cairo": "EG",
    "Africa/Accra": "GH",
  ...
  try {
    ...
    // 2. Try navigator.language (e.g. "en-NG", "fr-CI")
...
function makeTab(id: string, index: number): SearchTab {
  return {
    id, label: `Search ${index}`, mode: "keyword", country: DEFAULT_COUNTRY,
...
function exportToExcel(products: EnrichedProduct[], tabLabel: string) {
  const headers = [
    ...
    "Prohibited Item","Blacklisted Keyword","Restricted Brand","NG Seller Restriction",
    "MA Forbidden Brand","MA Fragrance-Only Brand","MA Seller Restriction","MA Prohibited Book Seller",
    "Invalid Variation","Counterfeit Flag","Suspicious Price",
  ];
  ...
  const rows = products.filter(Boolean).map((p) => [
    ...
    p.quality?.qualityScore != null && p.quality.qualityScore >= 0 ? p.quality.qualityScore : "",
    p.quality?.qualityScore != null && p.quality.qualityScore >= 0 ? p.quality.criticalIssues : "",
    p.quality?.qualityScore != null && p.quality.qualityScore >= 0 ? p.quality.highIssues : "",
    p.quality?.qualityScore != null && p.quality.qualityScore >= 0 ? p.quality.mediumIssues : "",
    p.quality?.qualityScore != null && p.quality.qualityScore >= 0 ? p.quality.lowIssues : "",
    p.quality?.qualityScore != null && p.quality.qualityScore >= 0 ? p.quality.totalIssues : "",
    p.quality?.qualityScore != null && p.quality.qualityScore >= 0 ? p.quality.topIssues?.join(" | ") ?? "" : "",
    ...
    flag(p.quality,"hasNonSquareImage"),
    ...
    flag(p.quality,"hasCategoryIssue"), flag(p.quality,"hasProhibitedItem"),
    flag(p.quality,"hasBlacklistedKeyword"), flag(p.quality,"hasRestrictedBrand"),
    flag(p.quality,"hasNGSellerRestriction"),
    flag(p.quality,"hasMAForbiddenBrand"), flag(p.quality,"hasMAFragranceOnlyBrand"),
    flag(p.quality,"hasMASellerRestriction"), flag(p.quality,"hasMAProhibitedBookSeller"),
    flag(p.quality,"hasInvalidVariation"),
  ...
  ws["!cols"] = [
    ...
    {wch:14},{wch:14},{wch:20},{wch:18},{wch:18},{wch:18},{wch:20},{wch:22},
    {wch:20},{wch:22},{wch:22},{wch:26},{wch:18},{wch:16},{wch:16},
    {wch:20},{wch:14},{wch:16},{wch:16},{wch:18},{wch:22},{wch:24},{wch:22},{wch:26},{wch:20},{wch:18},{wch:16},
  ];
...
export default function ProductSearchPage() {
  ...
  // Per-tab cancel token: set to true to abort an in-progress search
  ...
  const removeTab = (id: string, e: React.MouseEvent) => {
    ...
    cancelRefs.current[id] = true; // cancel any in-flight search
  ...
  // Includes silent retry for Render cold-start / spin-down delays:
  // if the request fails with a network error (server waking up), we wait 8s
  // and retry up to 3 times, keeping the spinner visible throughout.
      ...
      try {
        ...
        if (res.ok && res.summary) {
          ...
          quality = {
            ...
            hasNonSquareImage: hasType("non_square_image"),
            ...
            hasProhibitedItem: hasType("prohibited_item"),
            hasBlacklistedKeyword: hasType("blacklisted_keyword"),
            hasRestrictedBrand: hasType("restricted_brand"),
            hasNGSellerRestriction: hasType("ng_seller_restriction"),
            hasMAForbiddenBrand: hasType("ma_forbidden_brand"),
            hasMAFragranceOnlyBrand: hasType("ma_fragrance_only_brand"),
            hasMASellerRestriction: hasType("ma_seller_restriction"),
            hasMAProhibitedBookSeller: hasType("ma_prohibited_book_seller"),
            hasCounterfeitFlag: hasType("counterfeit_indicator"),
            hasSuspiciousPrice: hasType("suspicious_price"),
            hasInvalidVariation: hasType("invalid_variation"),
          };
        } else {
          // Check returned an error — mark quality as failed but don't export 0/100
          quality = {
            qualityScore: -1, criticalIssues: 0, highIssues: 0,
            ...
            hasLowResImages: false, hasNonSquareImage: false,
            hasEmptyDescription: false, hasThinDescription: false,
            hasMuddledDescription: false, hasRepeatedDescription: false, hasMissingDescImages: false,
            hasNamingIssue: false, hasCategoryIssue: false,
            hasProhibitedItem: false, hasBlacklistedKeyword: false, hasRestrictedBrand: false,
            hasNGSellerRestriction: false,
            hasMAForbiddenBrand: false,
            hasMAFragranceOnlyBrand: false,
            hasMASellerRestriction: false,
            hasMAProhibitedBookSeller: false,
            hasCounterfeitFlag: false, hasSuspiciousPrice: false, hasInvalidVariation: false,
          };
        }
        setTabs((prev) => prev.map((t) => {
          if (t.id !== tabId) return t;
        ...
        lastErr = err;
        // If it looks like a network/cold-start error and we have retries left,
        // wait silently and try again — keep the spinner showing, show no error.
        if (isNetworkError(err) && attempt < MAX_RETRIES) {
          await new Promise(res => setTimeout(res, RETRY_DELAY_MS));
          continue;
        }
        // Non-network error, or retries exhausted — surface the error
        break;
      }
    }

    // All retries failed — mark error
    setTabs((prev) => prev.map((t) => {
      if (t.id !== tabId) return t;
      const products = [...t.products];
      products[productIdx] = { ...products[productIdx], qualityChecking: false, qualityError: lastErr instanceof Error ? lastErr.message : "Failed" };
      return { ...t, products };
    }));
  }, [utils]);
  ...
  const handleSearch = async (tabId: string) => {
    ...
    // Reset cancel token for this tab
    cancelRefs.current[tabId] = false;

    const targetPages = Math.max(1, parseInt(tab.pages) || 1);

    // Silent retry for Render cold-start — same logic as runQualityCheck
    const RETRY_MAX = 3;
    ...
    const isNetworkErr = (e: unknown) => {
      ...
      return m.includes("failed to fetch") || m.includes("network") ||
             m.includes("econnrefused") || m.includes("load failed") ||
             m.includes("unexpected end") || m.includes("504") || m.includes("fetch");
    };
    ...
    try {
      if (tab.mode === "keyword") {
        ...
        for (let p = 1; p <= targetPages; p++) {
          if (cancelRefs.current[tabId]) return; // country changed — abort silently
          const result = await fetchWithRetry(() => utils.productSearch.byKeyword.fetch({ keyword: tab.keyword.trim(), country: tab.country, page: p }));
          if (cancelRefs.current[tabId]) return;
        ...
        for (let p = 1; p <= targetPages; p++) {
          ...
          const result = await fetchWithRetry(() => utils.productSearch.byUrl.fetch({ url: fetchUrl, country: tab.country }));
          if (cancelRefs.current[tabId]) return;
          if (result.products && result.products.length > 0) {
            allProducts = [...allProducts, ...(result.products as CatalogProduct[])];
            updateTab(tabId, { products: allProducts, progress: Math.round((p / targetPages) * 100) });
        ...
        // SKU — batch 5 at a time (mirrors testfinder page-by-page pattern)
        ...
        for (let b = 0; b < skus.length; b += SKU_BATCH_SIZE) {
          if (cancelRefs.current[tabId]) return; // country changed — abort silently
          const batch = skus.slice(b, b + SKU_BATCH_SIZE);
          const result = await fetchWithRetry(() =>
            utils.productSearch.bySku.fetch({ skus: batch, country: tab.country })
          );
          if (cancelRefs.current[tabId]) return;
          const batchProducts = (result.products || []) as CatalogProduct[];
          allProducts = [...allProducts, ...batchProducts];
          const progress = Math.round(Math.min(b + SKU_BATCH_SIZE, skus.length) / skus.length * 100);
          updateTab(tabId, { products: allProducts, progress });
        }

        stopTimer(tabId);
        updateTab(tabId, { products: allProducts, isSearching: false, progress: 100 });
        if (allProducts.length === 0) { toast.info("No products found for those SKUs"); }
        else { toast.success(`Found ${allProducts.length} product${allProducts.length !== 1 ? "s" : ""}`); runAllQualityChecks(tabId, allProducts); }
      }
    } catch (err) {
      if (cancelRefs.current[tabId]) return; // cancelled — don't show error
      stopTimer(tabId);
  ...
  function getFiltered(tab: SearchTab): EnrichedProduct[] {
    return tab.products.filter(Boolean).filter((p) => {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-200 px-6 py-4 sticky top-0 z-20">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-orange-600 transition-colors border border-gray-200 hover:border-orange-300 rounded-lg px-3 py-1.5"
          >
            <MessageSquare size={13} /> QC Assistant
          </button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          {tabs.map((tab) => {
            const safeProducts = tab.products.filter(Boolean);
            const filtered = getFiltered(tab);
            const uniqueBrands = Array.from(new Set(safeProducts.map((p) => p.brand).filter(Boolean)));
            const uniqueSellers = Array.from(new Set(safeProducts.map((p) => p.seller).filter(Boolean)));
            const uniqueCategories = Array.from(new Set(safeProducts.map((p) => p.category).filter(Boolean)));
            const uniqueTags = Array.from(new Set(safeProducts.flatMap((p) => Array.isArray(p.tags) ? p.tags : []).filter(Boolean))).sort();
            const checkedCount = safeProducts.filter((p) => p.quality && p.quality.qualityScore >= 0).length;
            const avgScore = checkedCount > 0
              ? Math.round(safeProducts.filter((p) => p.quality && p.quality.qualityScore >= 0).reduce((s, p) => s + (p.quality!.qualityScore || 0), 0) / checkedCount)
              : null;
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
                  <div className="flex flex-wrap gap-3 items-end">
                    <div>
                      ...
                      <Select value={tab.country} onValueChange={(v) => {
                        // Cancel any in-progress search for this tab
                        if (tab.isSearching) {
                          cancelRefs.current[tab.id] = true;
                          stopTimer(tab.id);
                        }
                        updateTab(tab.id, { country: v, products: [], error: null, progress: 0, isSearching: false });
                      }}>
                                ...
                                <tr key={`${product.sku}-${idx}`} className="hover:bg-gray-50 transition-colors">
                                  ...
                                  <td className="px-4 py-2">
                                    {product.qualityChecking
                                      ...
                                      : product.quality
                                        ? product.quality.qualityScore < 0
                        ...
                        {filtered.map((product, idx) => {
                          ...
                          <div key={`${product.sku}-${idx}`} className="bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-md transition-shadow group relative">
                            ...
                            <div className="relative aspect-square bg-gray-50">
                              ...
                              {product.quality && product.quality.qualityScore >= 0 && (
                              ...
                              {product.quality && product.quality.qualityScore < 0 && (
                                <div className="absolute bottom-1.5 left-1.5 text-[10px] font-bold px-1.5 py-0.5 rounded-full border bg-red-100 text-red-700 border-red-200">
                                  Failed
                                </div>
                              )}
                              {product.qualityChecking && (
