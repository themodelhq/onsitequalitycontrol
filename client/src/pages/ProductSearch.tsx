...
interface SearchTab {
  ...
  filterBrand: string; filterSeller: string; filterCategory: string; filterMinRating: string;
  view: "table" | "grid";
}
...
function makeTab(id: string, index: number): SearchTab {
  return {
    ...
    filterBrand: "all", filterSeller: "all", filterCategory: "all", filterMinRating: "0",
    view: "table",
  };
}
...
export default function ProductSearchPage() {
  ...
  // Mutations for scraping — POST requests avoid platform GET-timeout issues.
  // We store mutateAsync in refs so the callbacks always see the latest version
  ...
  const byKeywordRef = useRef(byKeywordMutation.mutateAsync);
  const byUrlRef = useRef(byUrlMutation.mutateAsync);
  const bySkuRef = useRef(bySkuMutation.mutateAsync);
  const checkUrlRef = useRef(checkUrlMutation.mutateAsync);
  byKeywordRef.current = byKeywordMutation.mutateAsync;
  byUrlRef.current = byUrlMutation.mutateAsync;
  bySkuRef.current = bySkuMutation.mutateAsync;
  checkUrlRef.current = checkUrlMutation.mutateAsync;
  ...
  const runQualityCheck = useCallback(async (tabId: string, productIdx: number, productUrl: string, country: string) => {
    ...
    try {
      const res = await checkUrlRef.current({ url: productUrl, country });
      ...
      if (res.ok && res.summary) {
        ...
        quality = {
          ...
          topIssues: (res.issues as any[]).slice(0,3).map((i:any) => `[${i.severity}] ${i.message}`),
        };
      } else {
        quality = {
    ...
    try {
      if (tab.mode === "keyword") {
        ...
        for (let p = 1; p <= targetPages; p++) {
          const result = await byKeywordRef.current({ keyword: tab.keyword.trim(), country: tab.country, page: p });
        ...
        for (let p = 1; p <= targetPages; p++) {
          ...
          const result = await byUrlRef.current({ url: fetchUrl, country: tab.country });
          if (result.products && result.products.length > 0) {
            allProducts = [...allProducts, ...(result.products as CatalogProduct[])];
            updateTab(tabId, { products: allProducts, progress: Math.round((p / targetPages) * 100) });
          }
        ...
        const result = await bySkuRef.current({ skus, country: tab.country });
                  ...
                  <>
                      ...
                      <div className="flex gap-2 flex-wrap ml-auto items-center">
                        {uniqueBrands.length > 0 && (
                          <Select value={tab.filterBrand} onValueChange={(v) => updateTab(tab.id, { filterBrand: v })}>
                            <SelectTrigger className="h-8 text-xs w-32"><SelectValue placeholder="Brand" /></SelectTrigger>
                        ...
                        {uniqueSellers.length > 0 && (
                          <Select value={tab.filterSeller} onValueChange={(v) => updateTab(tab.id, { filterSeller: v })}>
                            <SelectTrigger className="h-8 text-xs w-32"><SelectValue placeholder="Seller" /></SelectTrigger>
                        ...
                        <div className="flex border border-gray-200 rounded-lg p-0.5">
                          <button
                            onClick={() => updateTab(tab.id, { view: "table" })}
                            className={`p-1.5 rounded-md transition-all ${tab.view === "table" ? "bg-orange-500 text-white" : "text-gray-400 hover:text-gray-600"}`}
                            title="Table view"
                          >
                            <List size={14} />
                          </button>
                          <button
                            onClick={() => updateTab(tab.id, { view: "grid" })}
                            className={`p-1.5 rounded-md transition-all ${tab.view === "grid" ? "bg-orange-500 text-white" : "text-gray-400 hover:text-gray-600"}`}
                    ...
                    {filtered.length === 0 && tab.products.length > 0 && (
                      <div className="bg-white border border-gray-200 rounded-xl p-10 text-center">
                        ...
                        <button onClick={() => updateTab(tab.id, { filterBrand:"all", filterSeller:"all", filterCategory:"all", filterMinRating:"0" })}
