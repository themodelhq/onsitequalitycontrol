      ...
      try {
        ...
        if (res.ok && res.summary) {
          ...
          quality = {
            ...
            lowIssues: 0,
            totalIssues: 0,
            topIssues: [res.errorMessage || "Check failed"],
          };
        }
  ...
  const handleSearch = async (tabId: string) => {
    ...
    try {
      ...
      if (tab.mode === "keyword") {
        ...
        const res = await utils.productSearch.byKeyword.fetch({
        ...
        const res = await utils.productSearch.byUrl.fetch({
        ...
        const res = await utils.productSearch.bySku.fetch({
          skus,
          country: tab.country,
        });
        if (res.error) throw new Error(res.error);
