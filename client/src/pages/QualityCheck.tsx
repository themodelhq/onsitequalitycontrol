...
export default function QualityCheckPage() {
  ...
  // Keep a ref so runCheck (called inside loops) always sees the latest mutateAsync
  const checkUrlMutateRef = useRef(checkUrlMutation.mutateAsync);
  checkUrlMutateRef.current = checkUrlMutation.mutateAsync;

  // ── Core check function ───────────────────────────────────────────────────

  async function runCheck(url: string): Promise<CheckResult> {
    const res = await checkUrlMutateRef.current({
      url: url.trim(),
      country,
    });
