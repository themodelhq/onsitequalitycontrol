...
import { getInMemoryBlacklistedKeywords, getInMemoryProhibitedItems, getInMemoryRestrictedBrands, getInMemoryNamingFormatByCategory } from "../referenceData";
import { getNGSellerRules, isApprovedSellerForRule, isApprovedBrandForRule, getMAForbiddenBrands, getMAFragranceOnlyBrands, getMASellerRules, getMAProhibitedBookSellers } from "../referenceData";
...
export async function validateProductName(
  ...
  let format: Awaited<ReturnType<typeof getNamingFormatByCategory>>;
  try { format = await getNamingFormatByCategory(category); }
  catch { format = getInMemoryNamingFormatByCategory(category) as any; }
...
export async function checkProhibitedItems(
  ...
  // Wrap each DB call in its own try/catch so a DB connection failure
  // falls back to the in-memory reference data and never crashes the check.
  let blacklistedKeywords: Awaited<ReturnType<typeof getBlacklistedKeywords>>;
  try { blacklistedKeywords = await getBlacklistedKeywords(); }
  catch { blacklistedKeywords = getInMemoryBlacklistedKeywords() as any; }

  let prohibitedItems: Awaited<ReturnType<typeof getProhibitedItems>>;
  try { prohibitedItems = await getProhibitedItems(); }
  catch { prohibitedItems = getInMemoryProhibitedItems() as any; }

  let restrictedBrands: Awaited<ReturnType<typeof getRestrictedBrands>>;
  try { restrictedBrands = await getRestrictedBrands(); }
  catch { restrictedBrands = getInMemoryRestrictedBrands() as any; }
 ...
 *
 * Checks performed:
 *   1. Explicit counterfeit language (replica, fake, knockoff, first copy, etc.)
 *   2. Suspicious pricing for restricted/luxury brands — if the brand is on the
 *      QC-for-fakes list AND the price is suspiciously low vs the known floor,
 *      this signals either a fake product or a stolen/grey-market item.
 *   3. Misspelled brand name in product title — sellers often misspell brand names
 *      to evade keyword filters (Adiddas, Nikee, Addidas, etc.)
 *   4. Image/listing signals — "7 days return", "no brand", "inspired", etc.
 *   5. Known fake product name patterns (for QC-for-fakes brands)
 */
export function detectCounterfeitIndicators(
  productName: string,
  description: string,
  brand?: string,
  price?: number,
  ...
  const issues: string[] = [];
  const details: Record<string, any> = { indicators: [] };

  const searchText = `${productName} ${description}`.toLowerCase();
  const nameLower = productName.toLowerCase();

  // ── 1. Explicit counterfeit language ─────────────────────────────────────
  ...
  // ── 2. Brand name misspellings (common fake-evasion tactic) ──────────────
  // Pairs: [canonicalBrand, misspellings regex]
  const BRAND_MISSPELLINGS: Array<[string, RegExp]> = [
  ...
  // ── 3. QC-for-fakes brand + suspicious price check ───────────────────────
  // These are brands from our restricted list with restrictionType = 'verified_sellers_only'.
  // A genuinely low price on a premium brand product is a strong counterfeit signal.
  // Price floors in USD — approximate minimum for genuine products.
  const QC_BRAND_PRICE_FLOORS_USD: Record<string, number> = {
    "apple": 80,       // AirPods start ~$80, iPhones ~$300+
    "samsung": 30,     // Galaxy phones start ~$100, accessories lower
    "nike": 15,        // Genuine Nike shoes start ~$50, accessories ~$15
    "adidas": 15,
    "gucci": 150,
    "louis vuitton": 200,
    "rolex": 3000,
  ...
  // Approximate NGN/USD rate for price floor comparison
  ...
  const rateMap: Record<string, number> = {
    NGN: APPROX_NGN_PER_USD, KES: APPROX_KES_PER_USD,
    GHS: APPROX_GHS_PER_USD, EGP: APPROX_EGP_PER_USD,
    USD: 1, EUR: 0.92, GBP: 0.79,
    MAD: 10, DZD: 135, TND: 3, XOF: 600, UGX: 3750,
  };
  ...
  if (price && price > 0 && brand) {
    ...
    if (floorUSD) {
      ...
      // Flag if price is less than 30% of the known floor for this brand
      if (priceUSD < floorUSD * 0.30) {
        const pct = Math.round((priceUSD / floorUSD) * 100);
        issues.push(
          `Counterfeit indicator: Price (${price.toLocaleString()} ${cur} ≈ $${priceUSD.toFixed(0)} USD) ` +
          `is only ${pct}% of the typical genuine "${brand}" product floor ($${floorUSD} USD) — ` +
          `possible counterfeit or fake`
        );
        details.indicators.push(`Suspicious price for ${brand}: ${pct}% of floor`);
      }
    }

    // Also check for brand name in product name when brand field says something different
    // e.g. brand="Generic" but product name mentions "Nike" — likely counterfeit
  ...
  // ── 4. Additional listing-level signals ──────────────────────────────────
  // Note: "Grade A/B" is intentionally excluded here — while it can indicate
  // refurbished phones, it also legitimately appears in leather/material quality
  // descriptions (e.g. "Grade A Leather", "Grade B Synthetic") for fashion products,
...
/**
 * Check Nigeria-specific seller and brand rules.
 *
 * Verifies that the seller/brand combination is approved to list certain
 * products in Nigeria. Returns issues for each violated rule.
 */
export function checkNigeriaSpecificRules(
  productName: string,
  category: string,
  brand: string,
  seller: string
  ...
  const issues: string[] = [];
  const details: Record<string, any> = { violations: [] };
  ...
  for (const rule of rules) {
    // Brand check: if brands[] is empty, applies to any brand
    const brandMatch = rule.brands.length === 0 ||
      rule.brands.some(b => brandLower.includes(b.toLowerCase()));

    if (!brandMatch) continue;

    // Category and product keyword checks.
    // Empty arrays mean "no restriction on this dimension" — NOT "match everything".
    // We require at least one positive signal from category OR product name.
    // If both are empty, the rule is purely brand-gated and triggers on brand match alone.
    const hasCatKeywords  = rule.categoryKeywords.length > 0;
    const hasNameKeywords = rule.productKeywords.length > 0;

    let ruleTriggered: boolean;
    if (!hasCatKeywords && !hasNameKeywords) {
      // Pure brand rule — brand match alone is sufficient
      ruleTriggered = true;
    } else {
      ...
      // Trigger only when at least one dimension matches
      ruleTriggered = catMatch || nameMatch;
    }

    if (!ruleTriggered) continue;

    // For brand-gated rules (e.g. power bank brands)
...
/**
 * Morocco (MA) specific quality checks.
 * Runs only when country === "MA".
 *
 * Checks (in order):
 *  1. Brand is in MA_FORBIDDEN_BRANDS  → High  "ma_forbidden_brand"
 *  2. Brand is in MA_FRAGRANCE_ONLY_BRANDS but category is NOT Fragrances → High "ma_fragrance_only_brand"
 *  3. Brand is in MA_SELLER_RULES but seller is not on the approved list → High "ma_seller_restriction"
 *  4. Category is Books/book-related AND seller is in MA_PROHIBITED_BOOK_SELLERS → High "ma_prohibited_book_seller"
 */
export function checkMoroccoSpecificRules(
  productName: string,
  category: string,
  brand: string,
  seller: string
  ...
  // 1. Completely forbidden brands in MA
  ...
  // 2. Fragrance-only brands — allowed only when listed under Fragrances
  if (!isForbidden) {
    ...
    if (isFragranceOnly) {
      const isFragranceCategory =
        catLower.includes("fragrance") ||
        catLower.includes("perfume") ||
        catLower.includes("parfum") ||
        catLower.includes("eau de");
      if (!isFragranceCategory) {
        issues.push(`MA Rule: Brand "${brand}" is restricted to the Fragrances category only in Morocco. This product is listed under "${category}"`);
        details.violations.push({ rule: "ma_fragrance_only_brand", brand, category, seller });
      }
    }
  }

  // 3. Seller-gated brands — brand may only be listed by approved sellers
  ...
  // 4. Prohibited book sellers — Expresse Shop and Shopear cannot list books
  if (seller) {
    ...
    if (isProhibitedSeller) {
      const isBookCategory =
        ...
        catLower.includes("livre") ||
        catLower.includes("roman") ||
        catLower.includes("novel") ||
        catLower.includes("literature");
      // Also flag if product name strongly indicates a book
      const nameLower = productName.toLowerCase();
      const nameIndicatesBook =
        nameLower.includes(" by ") ||
        nameLower.includes("(auteur)") ||
        nameLower.includes("edition") ||
        nameLower.includes("novel") ||
        nameLower.includes(" tome ") ||
        nameLower.includes(" vol.") ||
        nameLower.includes("livre") ||
        nameLower.includes("roman");
      if (isBookCategory || nameIndicatesBook) {
        issues.push(`MA Rule: Seller "${seller}" is not permitted to list books on Jumia Morocco`);
        details.violations.push({ rule: "ma_prohibited_book_seller", brand, seller, category });
      }
    }
  }
