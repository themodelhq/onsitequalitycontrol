...
export function checkNigeriaSpecificRules(
  ...
  for (const rule of rules) {
    ...
    const catMatch = rule.categoryKeywords.length === 0 ||
      rule.categoryKeywords.some(k => catLower.includes(k.toLowerCase()));

    const nameMatch = rule.productKeywords.length === 0 ||
      rule.productKeywords.some(k => nameLower.includes(k.toLowerCase()));

    // Rule triggers if brand matches AND (category OR product name matches)
    const ruleTriggered = brandMatch && (catMatch || nameMatch);
