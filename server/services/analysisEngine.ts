...
import {
  ...
  validateCategory,
  checkNigeriaSpecificRules,
  checkMoroccoSpecificRules,
...
export async function runQualityChecksOnly(
  ...
  for (let i = 0; i < imageUrls.length; i++) {
    ...
    if (analysis.isNonSquare && analysis.width && analysis.height) {
      ...
      issues.push({
        ...
        severity: "medium",
        message: `Image ${i + 1} is not square (${analysis.width}×${analysis.height}, ratio ${ratio}) — Jumia requires 1:1 square images`,
  ...
  // 6. Counterfeit indicators (enhanced — includes price-floor and brand-mismatch checks)
  const counterfeitValidation = detectCounterfeitIndicators(
    product.name,
    product.description || "",
    product.brand || "",
    typeof product.price === "number" ? product.price : parseFloat(product.price) || 0,
    product.currency || country === "NG" ? "NGN"
      : country === "KE" ? "KES"
      : country === "GH" ? "GHS"
      : country === "EG" ? "EGP"
      : country === "MA" ? "MAD"
  ...
  // 8. Nigeria-specific seller/brand rules
  ...
  // 10. Morocco-specific rules (forbidden brands, fragrance-only brands, seller restrictions, prohibited book sellers)
  if (country === "MA") {
    ...
    if (!maValidation.isValid) {
      for (const viol of (maValidation.details?.violations || [])) {
        const ruleType = viol.rule as string;
        issues.push({
          type: ruleType,
          severity: "high",
          message: maValidation.issues.find((m: string) => m.includes(viol.brand || viol.seller)) || maValidation.issues[0],
          details: viol,
        });
      }
    }
  ...
  // 9. Variation check — only Fashion products may have variations/simples
...
function generateRecommendation(issueType: string): string {
  const recommendations: Record<string, string> = {
    ...
    non_square_image:
      "Replace image with a square (1:1 ratio) version. Jumia requires all product images to be square — rectangular images are rejected or displayed incorrectly",
    invalid_variation:
      "Remove size/colour variations (simples) from this listing. Variations are only permitted for Fashion category products on Jumia",
    non_square_image:
      "Replace with square (1:1 ratio) images — Jumia requires images that are perfectly square and fill the entire canvas",
    poor_description:
    ...
    invalid_variation:
      "Remove size/colour variations from this product — variations (simples) are only permitted for Fashion category products on Jumia",
  };
