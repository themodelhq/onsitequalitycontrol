...
function sampleJPEGBackground(view: Uint8Array, hadFillWhite = false): string {
  ...
  // the image, so the product was uploaded with a white (or near-white) background.
  // We only flag non-white if mid-range bytes overwhelmingly dominate (> 55%),
  // which only happens for images with clearly coloured or dark backgrounds.
  // Products like shoes, phones, accessories on white → midRatio typically 0.25–0.40
  // and are correctly passed as white. Default is always white for fill(white) images.
  if (hadFillWhite) {
    return midRatio > 0.55 ? "#808080" : "#FFFFFF";
  }
