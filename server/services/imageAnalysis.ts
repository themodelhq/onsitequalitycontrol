...
function sampleJPEGBackground(view: Uint8Array, hadFillWhite = false): string {
  ...
  // When the original CDN URL had fill(white), the seller uploaded an image that
  // Jumia was already padding with white. This means the product itself likely has
  // a white or near-white background. We use more lenient thresholds:
  // — strong non-white evidence required before flagging
  // — ambiguous results default to white (not non-white)
  if (hadFillWhite) {
    if (stuffedRatio > 0.02 || highRatio > 0.20) return "#FFFFFF";
    if (midRatio > 0.50 && stuffedRatio < 0.01) return "#808080"; // very strong non-white
    return "#FFFFFF"; // default to white when ambiguous and fill(white) was present
  }
  ...
  if (midRatio > 0.25 && stuffedRatio < 0.03) return "#808080";
...
function samplePNGBackground(view: Uint8Array, _width: number, _height: number, hadFillWhite = false): string {
  ...
  if (hadFillWhite) {
    if (highRatio > 0.30) return "#FFFFFF";
    if (highRatio > 0.15) return "#F0F0F0";
    return "#808080"; // very strong non-white needed to flag when fill(white) was set
  }
