...
function sampleJPEGBackground(view: Uint8Array, hadFillWhite = false): string {
  ...
  // (> 42%) AND the high-byte count is very low (< 10%).
  // A product on a white background — even a dark shoe or black rack — always
  // produces some high bytes (hr ≈ 0.10+) from the large white area.
  // A product on a coloured/lifestyle background has almost no high bytes (hr < 0.07).
  // This two-factor check correctly separates them.
  if (hadFillWhite) {
    return (midRatio > 0.42 && highRatio < 0.10) ? "#808080" : "#FFFFFF";
  }
