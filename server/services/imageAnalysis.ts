...
function sampleJPEGBackground(view: Uint8Array, hadFillWhite = false): string {
  ...
  // the image. We use a lenient threshold: only flag non-white when mid-range bytes
  // clearly dominate (> 42%), which handles dark/coloured backgrounds while correctly
  // passing products like shoes on white (midRatio typically 0.25–0.40).
  if (hadFillWhite) {
    return midRatio > 0.42 ? "#808080" : "#FFFFFF";
  }
