...
export async function createProduct(userId: number, product: any) {
  const db = await getDb();
  if (!db) {
    // DB unavailable — return a stub result so analysis can still proceed
...
import { getInMemoryProhibitedItems, getInMemoryBlacklistedKeywords, getInMemoryRestrictedBrands, getInMemoryNamingFormatByCategory } from "./referenceData";
