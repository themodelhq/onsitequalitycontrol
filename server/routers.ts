...
export const appRouter = router({
  ...
  analysis: router({
    ...
    uploadAndAnalyze: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        // Use authenticated user id, or 0 for guest (no login required)
     ...
     */
    getProductResults: publicProcedure
    ...
    getProducts: publicProcedure
      ...
      .query(async ({ ctx, input }) => {
        const userId = ctx.user?.id ?? 0;
        const products = await getProductsByUserId(userId, input.limit, input.offset);
        return products;
      }),
  }),
  ...
  export: router({
    ...
    getExportData: publicProcedure
      ...
      .query(async ({ ctx, input }) => {
        const userId = ctx.user?.id ?? 0;
        const products = await getProductsByUserId(userId, 1000, 0);
