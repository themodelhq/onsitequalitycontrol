...
export const appRouter = router({
  ...
  //
  // Returns catalog-listing products (not full detail-page data).
  // Used by the new ProductSearch page to search, view and export results.

  productSearch: router({
    /**
     * Keyword search — fetches one or more catalog pages for a keyword.
     */
    ...
    /**
     * URL search — fetches one or more pages from a Jumia catalog/store URL.
     */
    byUrl: publicProcedure
      ...
      .mutation(async ({ input }) => {
        try {
            ...
            const fetchUrl =
              page === 1
                ...
                : input.url.includes("?")
                  ? `${input.url}&page=${page}`
                  : `${input.url}?page=${page}`;
            const { products, hasMore: more } = await fetchCatalogByUrl(
              fetchUrl,
              input.country
            );
          ...
          return { products: allProducts, hasMore, error: null };
        } catch (err) {
          return {
            products: [],
            hasMore: false,
    ...
    /**
     * SKU search — looks up each SKU individually via catalog search.
     */
  ...
  //
  // Scrapes a live product page and runs the full analysis pipeline without
  // writing anything to the database. Useful for ad-hoc quality audits.

  qualityCheck: router({
    /**
     * Scrape a single Jumia product URL and return quality issues.
    ...
    checkUrl: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        // Feed the scraped data into the existing analysis pipeline.
        // Pass descriptionHtml as description so countImagesInDescription works.
        const productInput = {
          sku: p.sku,
          name: p.name,
          brand: p.brand,
        ...
        // Add suspicious-price issue if the scraper flagged it
