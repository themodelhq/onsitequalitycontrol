...
**API calls failing from Netlify (`<!DOCTYPE` JSON error / 404 on /api routes):**

This is the most common issue. Two things must be done:

1. **Update `netlify.toml`** — replace `YOUR-RENDER-APP` with your actual Render service name:
   ```toml
   [[redirects]]
     from = "/api/*"
     to = "https://jumia-qa-api.onrender.com/api/:splat"   # ← your real Render URL
     status = 200
     force = true
   ```
   The `/api/*` redirect **must appear before** the `/*` SPA redirect — Netlify processes them in order.

2. **Add `ALLOWED_ORIGINS` to Render** — set it to your Netlify site URL:
   ```
   ALLOWED_ORIGINS=https://onsiteqc.netlify.app
   ```

3. **Redeploy both** Netlify (to pick up netlify.toml changes) and Render (to pick up env var).

> **Simplest alternative**: Deploy as a monolith on Render (Option B above) — no proxy needed, no CORS issues.
